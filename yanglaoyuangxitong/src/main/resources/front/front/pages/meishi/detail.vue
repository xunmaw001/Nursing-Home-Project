			...
			<view class="yinyuewai">
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.meishiName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('meishi', this.meishiId);
                    this.detail = res.data
					this.detail.meishiContent = this.detail.meishiContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('meishiYuyue', {
									yonghuId: _this.user.id,
									meishiId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/meishiYuyue/list');
							}
						}
					});
				},
