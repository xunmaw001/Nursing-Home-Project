	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.yishengOrderId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('yishengOrder', this.yishengOrderId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.yishengOrderPhoto ? this.detail.yishengOrderPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('yishengOrderYuyue', {
									yonghuId: _this.user.id,
									yishengOrderId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/yishengOrderYuyue/list');
							}
						}
					});
				},
