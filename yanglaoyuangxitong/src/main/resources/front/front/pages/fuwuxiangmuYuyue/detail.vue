	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.fuwuxiangmuYuyueId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('fuwuxiangmuYuyue', this.fuwuxiangmuYuyueId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.fuwuxiangmuYuyuePhoto ? this.detail.fuwuxiangmuYuyuePhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('fuwuxiangmuYuyueYuyue', {
									yonghuId: _this.user.id,
									fuwuxiangmuYuyueId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/fuwuxiangmuYuyueYuyue/list');
							}
						}
					});
				},
