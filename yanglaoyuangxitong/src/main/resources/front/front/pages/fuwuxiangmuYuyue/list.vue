<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        ...
        <view class="action">
            ...
            <view v-if="isAuth('fuwuxiangmuYuyue','新增')" @click="onAddTap()"
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.fuwuxiangmuYuyueName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('fuwuxiangmuYuyue','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                ...
                <view v-show="item.fuwuxiangmuName" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">服务项目名称</text>
                    <text style="width: 70%">{{item.fuwuxiangmuName}}</text>
                </view>
                <view v-show="item.fuwuxiangmuTypes" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">服务项目类型</text>
                    <text style="width: 70%">{{item.fuwuxiangmuValue}}</text>
                </view>
                <!-- 当前表 -->
                <view v-show="item.fuwuxiangmuYuyueTime" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">预约时间</text>
                    <text style="width: 70%">
                        <text v-if="item.fuwuxiangmuYuyueTime != null">{{item.fuwuxiangmuYuyueTime}}</text>
                        <text v-else></text>
                    </text>
                </view>
                 <view v-show="item.fuwuxiangmuYuyueYesnoValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">审核状态</text>
                     <text style="width: 70%">{{item.fuwuxiangmuYuyueYesnoValue}}</text>
                 </view>
                <view v-show="item.fuwuxiangmuYuyueYesnoText" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">审核回复</text>
                    <view style="width: 70%" v-html="item.fuwuxiangmuYuyueYesnoText"></view>
                </view>
                <view v-show="item.fuwuxiangmuYuyueShenheTime" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">审核时间</text>
                    <text style="width: 70%">
                        <text v-if="item.fuwuxiangmuYuyueShenheTime != null">{{item.fuwuxiangmuYuyueShenheTime}}</text>
                        <text v-else></text>
                    </text>
                </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('fuwuxiangmuYuyue','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('fuwuxiangmuYuyue','审核') && item.fuwuxiangmuYuyueYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let _this = this
                let res = await this.$api.page(`fuwuxiangmuYuyue`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('fuwuxiangmuYuyue', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.fuwuxiangmuYuyueShenheTime=shijian;
                            await _this.$api.update(`fuwuxiangmuYuyue`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
                ...
                let res = await this.$api.page(`fuwuxiangmuYuyue`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                })
				// 如果是第一页数据置空
