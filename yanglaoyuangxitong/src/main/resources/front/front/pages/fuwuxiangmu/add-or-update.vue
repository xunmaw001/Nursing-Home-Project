        ...
        <form class="app-update-pv">
            <view
                ...
                <input   disabled
                         ...
                         :disabled="ro.fuwuxiangmuName" type="text" v-model="ruleForm.fuwuxiangmuName" placeholder="服务项目名称"></input>
            </view>
                <view :style='{"boxShadow":"0","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"rgba(228, 232, 230, 1)","margin":"0 0 0px 0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"100rpx"}'
                      class="cu-form-group">
                    <view :style='{"width":"160rpx","fontSize":"28rpx","color":"var(--publicMainColor)","textAlign":"left"}'
                          class="title">服务项目类型</view>
                    <picker @change="fuwuxiangmuTypesChange" :value="fuwuxiangmuTypesIndex" :range="fuwuxiangmuTypesOptions" range-key="indexName">
                        <view
                                :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"10rpx 2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
                                class="uni-input">{{ruleForm.fuwuxiangmuTypes?ruleForm.fuwuxiangmuValue:"请选择服务项目类型"}}</view>
                    </picker>
                </view>
                <view
                        ...
                        class="cu-form-group" @tap="fuwuxiangmuPhotoTap">
                    <view :style='{"width":"160rpx","fontSize":"28rpx","color":"var(--publicMainColor)","textAlign":"left"}'
                          class="title">服务项目封面</view>
                    <view class="right-input" style="padding:0;textAlign:left">
                        <image
                                ...
                                class="avator" v-if="ruleForm.fuwuxiangmuPhoto" :src="baseUrl+ruleForm.fuwuxiangmuPhoto" mode="aspectFill">
                        </image>
            ...
            <view
                ...
                <textarea :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"10rpx 2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx","height":"200rpx"}'
                          :disabled="ro.fuwuxiangmuContent" v-model="ruleForm.fuwuxiangmuContent" placeholder="服务项目介绍"/>
            </view>
...
<script>
    ...
    export default {
		...
		/*下拉框*/
			let fuwuxiangmuTypesParams = {
                page: 1,
                limit: 100,
                dicCode: 'fuwuxiangmu_types',
            }
			let fuwuxiangmuTypes = await this.$api.page(`dictionary`, fuwuxiangmuTypesParams);
			this.fuwuxiangmuTypesOptions = fuwuxiangmuTypes.data.list
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`fuwuxiangmu`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.fuwuxiangmuId){
                this.ruleForm.fuwuxiangmuId = options.fuwuxiangmuId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
				if ((!this.ruleForm.fuwuxiangmuName)) {
                    this.$utils.msg(`服务项目名称不能为空`);
                    return
                }
				if ((!this.ruleForm.fuwuxiangmuPhoto)) {
                    this.$utils.msg(`服务项目封面不能为空`);
                    return
                }
                if (this.ruleForm.id) {
                    await this.$api.update(`fuwuxiangmu`, this.ruleForm);
                } else {
                    await this.$api.save(`fuwuxiangmu`, this.ruleForm);
                }
