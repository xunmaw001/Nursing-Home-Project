...
//                            yishengEntity.setYishengNumber(Integer.valueOf(data.get(0)));   //挂号人数限制 要改的
//                            yishengEntity.setYishengNewmoney(data.get(0));                    //挂号价格 要改的
//                            yishengEntity.setYishengTime(sdf.parse(data.get(0)));          //在诊日期 要改的
...
//                            yishengEntity.setInsertTime(date);//时间
...
//    	ValidatorUtils.validateEntity(user);
    ...
    */
    ...
    public R gexingtuijian(@RequestParam Map<String, Object> params, HttpServletRequest request){
        ...
        PageUtils pageUtils = yishengOrderService.queryPage(params1);
        List<YishengOrderView> orderViewsList =(List<YishengOrderView>)pageUtils.getList();
        Map<Integer,Integer> typeMap=new HashMap<>();//购买的类型list
        for(YishengOrderView orderView:orderViewsList){
            Integer yishengTypes = orderView.getYishengTypes();
